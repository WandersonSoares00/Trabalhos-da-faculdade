...
/* Inicializa os caixas determinados por "qtd_caixas".*/
int c_inicializar(Caixa **c, int qtd_caixas){
    register int i;
    *c = (Caixa *) malloc((qtd_caixas + 1) * sizeof(Caixa)); /*Aloca uma posição adicional para indicar o final do vetor*/
    if(*c == NULL)
        return 0;
    
    (*c + qtd_caixas) -> oper = -1; /*Indica fim do vetor c*/
